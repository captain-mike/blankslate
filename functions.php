...
register_nav_menus( array( 'main-menu' => esc_html__( 'Main Menu', 'pizza' ) ) );
}
add_action( 'admin_notices', 'pizza_notice' );
function pizza_notice() {
$user_id = get_current_user_id();
$admin_url = ( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http' ) . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
$param = ( count( $_GET ) ) ? '&' : '?';
if ( !get_user_meta( $user_id, 'pizza_notice_dismissed_8' ) && current_user_can( 'manage_options' ) )
echo '<div class="notice notice-info"><p><a href="' . esc_url( $admin_url ), esc_html( $param ) . 'dismiss" class="alignright" style="text-decoration:none"><big>' . esc_html__( 'Ⓧ', 'pizza' ) . '</big></a>' . wp_kses_post( __( '<big><strong>📝 Thank you for using pizza!</strong></big>', 'pizza' ) ) . '<br /><br /><a href="https://wordpress.org/support/theme/pizza/reviews/#new-post" class="button-primary" target="_blank">' . esc_html__( 'Review', 'pizza' ) . '</a> <a href="https://github.com/tidythemes/pizza/issues" class="button-primary" target="_blank">' . esc_html__( 'Feature Requests & Support', 'pizza' ) . '</a> <a href="https://calmestghost.com/donate" class="button-primary" target="_blank">' . esc_html__( 'Donate', 'pizza' ) . '</a></p></div>';
}
add_action( 'admin_init', 'pizza_notice_dismissed' );
function pizza_notice_dismissed() {
$user_id = get_current_user_id();
if ( isset( $_GET['dismiss'] ) )
add_user_meta( $user_id, 'pizza_notice_dismissed_8', 'true', true );
}
